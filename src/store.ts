...
type AppStore = {
  ...
  addNeed: (text: string, category: NeedCategory, requestedBy: FamilyMember) => void
...
export const useAppStore = create<AppStore>()(persist((set) => ({
  ...
  selectedCalendarId: '',
  calendarCache: null,
  addNeed: (text, category, requestedBy) => set((state) => ({
    needs: [{ id: crypto.randomUUID(), text: text.trim(), category, requestedBy, createdAt: Date.now(), completedAt: null }, ...state.needs],
  })),
  ...
  toggleTask: (dateKey, taskId) => set((state) => {
    ...
    return {
      completedByDate: {
        ...
        [dateKey]: completed.includes(taskId) ? completed.filter((id) => id !== taskId) : [...completed, taskId],
      },
    }
  }),
