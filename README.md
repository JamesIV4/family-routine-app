...
The app opens School Prep on launch and switches back to it once each morning after 5:30 AM while the app is open. Routine checks reset by local calendar date. After 5 PM, School Prep shows tomorrow's list so clothes can be laid out the night before. The amber **NOW** marker moves through the deadline list. On the wall display, the next overdue task gives three gentle icon nudges during the active morning, then rests. The optional three-pulse screen alert uses a warm edge glow; dings are off until enabled. Alerts depend on the app staying open and awake.

Checking tasks gives a small celebratory bounce. Completing all 12 School Prep tasks, including clothes, shows an **All set!** message and the [canvas-confetti Realistic Look](https://www.kirilv.com/canvas-confetti/) burst once per routine date. Returning to a completed routine or undoing and rechecking it does not replay the confetti. New Needs and section changes have brief, gentle animations. The iPad's **Reduce Motion** preference disables decorative motion and confetti while keeping the completion message and all controls available.
